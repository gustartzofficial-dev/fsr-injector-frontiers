...
struct ViewInfo {
    ID3D11Resource* res = nullptr; // borrowed; never Release from state arrays
    TexInfo tex{};
...
uint64_t candidate_key(const char* kind, uint64_t draw, uint64_t dispatch, ID3D11PixelShader* ps, ID3D11ComputeShader* cs, int score) {
    uintptr_t a = reinterpret_cast<uintptr_t>(ps);
    uintptr_t b = reinterpret_cast<uintptr_t>(cs);
    return (static_cast<uint64_t>(a >> 4) ^ static_cast<uint64_t>(b >> 3) ^
            (draw << 16) ^ (dispatch << 32) ^ static_cast<uint64_t>(score) ^
...
void evaluate_candidate_locked(const char* kind, UINT a = 0, UINT b = 0, UINT c = 0) {
    ...
    for (const auto& r : g_state.rtvs) {
        if (!r.res || !screenish(r.tex.width, r.tex.height)) continue;
        has_output = true;
        DXGI_FORMAT f = effective_format(r.tex, r.view_fmt);
        if (f == DXGI_FORMAT_R8G8B8A8_UNORM || f == DXGI_FORMAT_B8G8R8A8_UNORM || hdr_like(f)) score += 2;
    }
    ...
    auto scan_srv = [&](const ViewInfo& s) {
        if (!s.res || !screenish(s.tex.width, s.tex.height)) return;
        ++srv_count;
        DXGI_FORMAT f = effective_format(s.tex, s.view_fmt);
        if (hdr_like(f)) {
            has_hdr = true;
            score += 3;
        }
        if (depth_resource_like(s.tex, f)) {
            has_depth = true;
            score += 3;
        }
        if (strong_velocity_format(f)) {
            has_strong_velocity = true;
            score += 4;
        } else if (weak_velocity_or_packed_format(f)) {
            has_weak_velocity = true;
            score += 1;
        }
        if (color_history_like(f)) {
            ++color_history_count;
            score += 1;
        }
    };

    for (const auto& s : g_state.ps_srvs) scan_srv(s);
    for (const auto& s : g_state.cs_srvs) scan_srv(s);

    // v2: Be strict. v1 spammed copy/upscale passes with only one R8 SRV.
    bool is_interesting = false;
    if (has_output && has_hdr && has_depth && color_history_count >= 2 && (has_strong_velocity || has_weak_velocity)) {
        is_interesting = true;
    }
    if (has_output && has_depth && has_strong_velocity && color_history_count >= 2) {
        is_interesting = true;
    }
    // Allow compute temporal/upscale path when there is no RTV but there are rich SRVs.
    if (!has_output && kind && kind[0] == 'D' && has_hdr && has_depth && color_history_count >= 2 && (has_strong_velocity || has_weak_velocity)) {
        is_interesting = true;
    }
    ...
    if (score < 11) return;
    if (g_candidate_counter >= 80) return;

    uint64_t key = candidate_key(kind, g_draw_counter, g_dispatch_counter, g_state.ps, g_state.cs, score);
    if (!g_seen_candidate_keys.insert(key).second) return;

    ++g_candidate_counter;
    LOGF("[taa-trace] candidate #%llu kind=%s draw=%llu dispatch=%llu args=%u,%u,%u score=%d hdr=%d depth=%d strongVelocity=%d weakVelocity=%d historyColor=%d srvs=%d ps=0x%p cs=0x%p",
         ...
         a, b, c, score, has_hdr ? 1 : 0, has_depth ? 1 : 0, has_strong_velocity ? 1 : 0, has_weak_velocity ? 1 : 0,
         color_history_count, srv_count, g_state.ps, g_state.cs);
    ...
    if (SUCCEEDED(hr) && desc && out_tex && *out_tex) {
        ...
        bool interesting = screenish(t.width, t.height) &&
            (hdr_like(t.fmt) || weak_velocity_or_packed_format(t.fmt) ||
             (t.bind & D3D11_BIND_DEPTH_STENCIL) || depth_resource_like(t, t.fmt));
        if (interesting && g_create_logged < 160) {
            ++g_create_logged;
...
void STDMETHODCALLTYPE hk_CSSetShader(ID3D11DeviceContext* ctx, ID3D11ComputeShader* shader, ID3D11ClassInstance* const* classes, UINT class_count) {
    {
        ...
        g_state.cs = shader;
        uintptr_t p = reinterpret_cast<uintptr_t>(shader);
        if (shader && g_seen_cs.insert(p).second && g_seen_cs.size() <= 96) {
...
void note_draw_locked(UINT a, UINT b, UINT c, const char* kind) {
    ...
    if (g_draw_counter == 1 || g_draw_counter == 1000 || (g_draw_counter % 5000) == 0) {
        LOGF("[taa-trace] progress draw=%llu dispatch=%llu candidates=%llu creates=%llu trackedTextures=%llu",
             static_cast<unsigned long long>(g_draw_counter), static_cast<unsigned long long>(g_dispatch_counter),
             static_cast<unsigned long long>(g_candidate_counter), static_cast<unsigned long long>(g_next_tex_id - 1),
...
void STDMETHODCALLTYPE hk_Dispatch(ID3D11DeviceContext* ctx, UINT x, UINT y, UINT z) {
    {
        ...
        if (g_dispatch_counter == 1 || g_dispatch_counter == 100 || (g_dispatch_counter % 1000) == 0) {
            LOGF("[taa-trace] dispatch-progress draw=%llu dispatch=%llu candidates=%llu creates=%llu trackedTextures=%llu",
                 static_cast<unsigned long long>(g_draw_counter), static_cast<unsigned long long>(g_dispatch_counter),
                 static_cast<unsigned long long>(g_candidate_counter), static_cast<unsigned long long>(g_next_tex_id - 1),
                 static_cast<unsigned long long>(g_textures.size()));
        }
...
bool install() {
    ...
    // D3D11 CS methods are late in the immediate context vtable.
    ok &= hook_one(ctx_vt[107], reinterpret_cast<void*>(&hk_CSSetShaderResources), reinterpret_cast<void**>(&g_orig_cs_srvs), "CSSetShaderResources");
    ok &= hook_one(ctx_vt[109], reinterpret_cast<void*>(&hk_CSSetShader), reinterpret_cast<void**>(&g_orig_cs_shader), "CSSetShader");

    ctx->Release();
    dev->Release();

    if (!ok) {
        LOGF("[taa-trace] install incomplete; logger may be partial");
    }

    LOGF("[taa-trace] v2 installed: strict TemporalUpscaler/TAA tracer");
    LOGF("[taa-trace] v2 candidate rule requires HDR/depth/history/velocity-like resources; early copy/upscale spam is ignored");
    LOGF("[taa-trace] purpose: identify HE2 TemporalUpscaler inputs: scene color, depth/stencil, history, velocity/motion vectors");
    return ok;
}
